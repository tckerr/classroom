...
export class NewGameFormComponent implements OnInit {
   ...
   constructor(private gamesService: GamesService, private notificationService: GameSummaryUpdateNotifierService) {
   ...
   public handleCreateGameResponse(result: GameSummary) {
      this.loading = false;
      this.notificationService.broadcastUpdate(result);
   }
